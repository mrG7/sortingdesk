...
** TODO [#A] Implement jasmine (unit) tests
...
** TODO Allow text items to be dismissed via the keyboard
** TODO Show an 'X' on each text item to allow it to be dismissed on mouse click
** TODO Enable dismissal of text items via a configurable keystroke
...
* Bugs [66%]
** TODO [#A] IE only: text items cannot be dropped onto primary bin
** TODO Fix error thrown when manual bin add cancelled

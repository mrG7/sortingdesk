...
(function (factory, root) {
    ...
    API.prototype.deleteFolder = function(folder) {
        var params = {annotator_id: folder.annotator},
            endpoint = ['folder', folder.id].join('/'),
            url = this.url(endpoint, params);
        return this.xhr.ajax('API.deleteFolder', {
            type: 'DELETE',
            url: url
        }).fail(function() {
            console.error('Failed to delete folder:', folder);
        });
    };

    API.prototype.deleteSubfolder = function(subfolder) {
        var params = {annotator_id: subfolder.folder.annotator},
            endpoint = [
                'folder', subfolder.folder.id, 'subfolder', subfolder.id
            ].join('/'),
            url = this.url(endpoint, params);
        return this.xhr.ajax('API.deleteSubfolder', {
            type: 'DELETE',
            url: url
        }).fail(function() {
            console.error('Failed to delete subfolder:', subfolder);
        });
    };

    API.prototype.deleteSubfolderItem = function(subfolder, content_id,
                                                 subtopic_id /* optional */) {
        var params = {annotator_id: subfolder.folder.annotator},
            endpoint = [
                'folder', subfolder.folder.id, 'subfolder', subfolder.id,
        ...
        var url = this.url(endpoint.join('/'), params);
        return this.xhr.ajax('API.deleteSubfolderItem', {
            type: 'DELETE',
            url: url
        }).fail(function() {
            console.error('Failed to delete subfolder item:',
                          subfolder, content_id, subtopic_id);
        });
    };

    API.prototype.renameFolder = function(src_folder, dest_folder) {
        var params = {annotator_id: src_folder.annotator},
            endpoint = ['folder', src_folder.id,
                        'rename', dest_folder.id].join('/'),
            url = this.url(endpoint, params);
        return this.xhr.ajax('API.renameFolder', {
            type: 'POST',
            url: url
        }).fail(function() {
            console.error('Failed to rename folder:', src_folder, dest_folder);
        });
    };

    API.prototype.renameSubfolder = function(src_subfolder, dest_subfolder) {
        var params = {annotator_id: src_subfolder.folder.annotator},
            endpoint = ['folder', src_subfolder.folder.id,
                        'subfolder', src_subfolder.id,
                        'rename', dest_subfolder.folder.id,
                        'subfolder', dest_subfolder.id].join('/'),
            url = this.url(endpoint, params);
        return this.xhr.ajax('API.renameSubfolder', {
            type: 'POST',
            url: url
        }).fail(function() {
            console.error('Failed to rename subfolder:',
                          src_subfolder, dest_subfolder);
        });
    };
